...
function track(id: string, from: [number, number], to: [number, number], n = 5): Track {
  ...
  return { id, points, lastSeen: 0, heading: 0, speedKmh: 0, info: null };
}
...
describe('analyze', () => {
  ...
  it('треки из одной точки пропускаются', () => {
    const res = analyze([{ id: 'a', points: [{ lat: 55, lon: 37 }], lastSeen: 0, heading: 0, speedKmh: 0, info: null }], bounds);
